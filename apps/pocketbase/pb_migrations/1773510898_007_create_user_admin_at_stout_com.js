...
migrate((app) => {
  ...
  record.set("position", "Administrador");
  record.set("role", "administrator");
  record.set("status", "active");
  record.set("hire_date", "2021-01-01");
  ...
  const record_sector_idLookup = app.findFirstRecordByFilter("sectors", "name='Administrativo' && unit_id.name='Limeira'");
  if (!record_sector_idLookup) { throw new Error("Lookup failed for sector_id: no record in 'sectors' matching \"name='Administrativo' && unit_id.name='Limeira'\""); }
