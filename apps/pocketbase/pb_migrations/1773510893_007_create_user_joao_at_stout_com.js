...
migrate((app) => {
  ...
  const record_sector_idLookup = app.findFirstRecordByFilter("sectors", "name='Cozinha' && unit_id.name='Limeira'");
  if (!record_sector_idLookup) { throw new Error("Lookup failed for sector_id: no record in 'sectors' matching \"name='Cozinha' && unit_id.name='Limeira'\""); }
  ...
  try {
    const record = app.findFirstRecordByData("users", "email", "joao@stout.com");
    app.delete(record);
  } catch (e) {
